  ...
  addNewGoals(goalData,callback) {
    async.waterfall([
      ...
      function(connection,callback) {
        var fatigue;
        let date = goalData.date.split("T0")[0];
        console.log(date);
        (goalData.fatigue == " " ) ? fatigue = "false" : fatigue = goalData.fatigue;
        rethinkdb.table('goal').insert({'blog': goalData.blog, 'description': goalData.description, 'category': goalData.category, 'date': date, 'sleep': goalData.sleep, 'fatigue': fatigue, 'exercise': goalData.exercise, 'meal': goalData.meal}
